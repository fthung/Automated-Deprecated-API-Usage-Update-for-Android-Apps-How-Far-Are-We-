...
the number of experiments reported is moderate, involving only API usage updates in 41 usage locations \jl{Do we like this
  sentence?  I felt that some transition was needed.}\ft{Added details about the moderate number}. In this work, we
replicate the evaluation of AppEvolve and assess whether its effectiveness
is generalizable. Given the set of APIs on which AppEvolve has been
...
mitigation strategy that solves 81.82\% of these failures by a simple
refactoring of the app code to better resemble the code in the examples. The refactoring usually involves assigning a target API method invocation and
arguments of the target API method into variables \jl{I suggest to drop
  this detail.  The sentence is long.  If we need this in the abstract, it
  can be a separate senetnce.}\ft{make it a separate sentence}.  Indeed, we have also seen such
transformations in the dataset distributed with the AppEvolve replication
package, as compared to the original source code from which this dataset is
derived.  Based on these findings, we propose some promising future
directions.
